...
def process_fedavg(clients, server, model, name):
    ...
    outfile = os.path.join(args.outpath + '/' + args.dataset,
                           f'{args.dataset}_fedavg_{model}_client{args.num_clients}_{filename}_300_{args.name}.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--outpath", default='./adddirichlet', type=str)
    ...
    for i in range(10):
        time = i + 1
        # 准备一个数据集并切分
        splitedData, df_stats = setup.prepareData_fingerprint(
