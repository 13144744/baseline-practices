...
def mess_up_dataset(unprocessed_data, num_noise_percent, use_edge_attr, dataname):
    ...
    #     x_size = d.x.size(0)
    #     all_x_size = all_x_size + x_size
    #     size_feat = d.x.size(1)
    #     for x_node in d.x:
    #         x_node = x_node.detach().numpy()
    #         x_list.append(x_node)
    #     if max_x_size < x_size:
    #         max_x_size = x_size
    #     if min_x_size > x_size:
    #         min_x_size = x_size
    #     all_edge_size = all_edge_size + d.edge_index.size(1)
    #     y_list.append(d.y.item())
    #     rd = d
    #     # rd = Data(edge_index=d.edge_index, edge_attr=d.edge_attr, x=d.x,  y=d.y, pos=d.pos)
    #     dataset.append(rd)
    # avg_connect = int(all_edge_size / all_x_size)
    # if dataname == "IMDB-BINARY":
    #     for i in range(num_noise):
    #         # fake x
    #         fake_num_nodes = random.randint(min_x_size, max_x_size)
    #         fake_nodes = np.arange(0, fake_num_nodes)
    #         fake_x = torch.randn(fake_num_nodes, size_feat)
    #         # fake edge_index
    #         real2fake = np.random.choice(fake_nodes, size=(fake_x.size(0), avg_connect)).reshape(-1)
    #         fake2real = np.repeat(fake_nodes, avg_connect, axis=-1).reshape(-1)
    #         fake_edgeindex = np.vstack([fake2real, real2fake])
    #         fake_edge_index = torch.tensor(fake_edgeindex)
    #         # fake edge_attr
    #         if use_edge_attr == None:
    #             fake_edge_attr = None
    #         else:
    #             fake_edge_attr = None
    #         # fake y
    #         actual_y = np.unique(y_list)
    #         fake_y = np.random.choice(actual_y, 1)
    #         fake_y = torch.tensor(fake_y)
    #         # fake pos
    #         fake_pos = None
    #         fd = Data(edge_index=fake_edge_index, edge_attr=fake_edge_attr, x=fake_x, y=fake_y, pos=fake_pos, num_nodes=fake_num_nodes)
    #         dataset.append(fd)
    #     return dataset
    # for i in range(num_noise):
    #     # fake x
    ...
    #     # fake pos
    #     fake_pos = None
    #     fd = Data(edge_index=fake_edge_index, edge_attr=fake_edge_attr, x=fake_x,  y=fake_y, pos=fake_pos)
    #     dataset.append(fd)
